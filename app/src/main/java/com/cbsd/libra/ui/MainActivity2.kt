...
import com.cbsd.libra.model.Position
...
class MainActivity2 : AppCompatActivity() {
    ...
    private var viewPosition: Position? = null
                ...
                when (data.a) {
                    BleAction.DRAG -> {
                        ...
                        if (position.contains("#")){
                            ...
                            mainVideoContainer.translationX = positionData[0].toFloat()
                            mainVideoContainer.translationY = positionData[1].toFloat()
                            mainVideoContainer.scaleX = positionData[2].toFloat()
                            mainVideoContainer.scaleY = positionData[2].toFloat()
                            val lp = mainVideoView.layoutParams as FrameLayout.LayoutParams
                            lp.width = positionData[3].toInt()
                            lp.height = positionData[4].toInt()
...
//        if (ACache[this].getAsObject(ACacheConfig.POSITION2) != null)
//            viewPosition = ACache[this].getAsObject(
//                ACacheConfig.POSITION2
//            ) as Position
//        if (viewPosition != null) {
//            useCache()
//            LogUtils.d("viewPosition：" + GsonUtils.toJson(viewPosition))
//        } else {
//            viewPosition = Position(0F, 0F, 0F, 0, 0)
//            mainWidthSeekBar.progress = (DisplayUtils.dp2px(this, 214F).toDouble() / maxWidth * 100).toInt()
//            mainHeightSeekBar.progress = (DisplayUtils.dp2px(this, 120F).toDouble() / maxHeight * 100).toInt()
//        }
        mainVideoView.start()
    ...
    private var currentScaleX = 0
    private var currentScaleY = 0
...
//        startTimingGoneControlArea()
        ...
        /*
        设置seekBar progress = video.width / maxWidth
        eg: 100 / 1000, 则scaleX = 0.1
        默认width = 160dp， height = 240dp,详见dimens
         */
        val defaultWidth = resources.getDimensionPixelSize(R.dimen.default_width)
        val defaultHeight = resources.getDimensionPixelSize(R.dimen.default_height)
        val defaultScaleX = defaultWidth.toDouble() / maxWidth
        val defaultScaleY = defaultHeight.toDouble() / maxHeight
        mainWidthSeekBar.progress = (defaultScaleX * 100).toInt()
        mainHeightSeekBar.progress = (defaultScaleY * 100).toInt()
        LogUtils.d("maxWidth: $maxWidth, maxHeight: $maxHeight")
        LogUtils.d("defaultWidth: $defaultWidth, defaultHeight:$defaultHeight, defaultScaleX: $defaultScaleX, defaultScaleY: $defaultScaleY")
...
//                mainVideoView.toTheBottom(2)
        ...
        mainVideoContainer.addOnPositionChangedListener { translationX, translationY, scale ->
            LogUtil.d("onChanged:$scale")
            LogUtils.d("video width: ${mainVideoView.layoutParams.width}, height: ${mainVideoView.layoutParams.height}")
            val lp = mainVideoView.layoutParams as RelativeLayout.LayoutParams
            ACache[this].put(ACacheConfig.POSITION2, Position(translationX, translationY, scale, lp.width, lp.height))
            mainWidthSeekBar.progress *= scale.toInt()
            mainHeightSeekBar.progress *= scale.toInt()
